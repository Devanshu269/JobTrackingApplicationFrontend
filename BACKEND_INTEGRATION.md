...
> **Status (2026-08-10, UI fully integrated with the latest backend).**
> - **Backend:** every endpoint documented here is built, tested, and active.
> - **Frontend:** The frontend is fully migrated. All mock data has been deleted. The UI actively handles pagination (`page`, `size`), dynamic chart rendering using `getJobTrend()`, lightweight kanban drag/drop via `PATCH`, and secure authenticated S3/Cloudinary file downloads with dynamically injected original filenames.
